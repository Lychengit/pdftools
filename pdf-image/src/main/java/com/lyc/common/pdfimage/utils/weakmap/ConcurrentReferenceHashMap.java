		...
		return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
			...
			protected V execute( Reference<K, V> ref,  Entry<K, V> entry,  Entries<V> entries) {
				if (entry != null) {
					...
					if (oldValue instanceof ReferenceMapEntry) {
						((ReferenceMapEntry) oldValue).setEntry(entry);
					}
					return oldValue;
				}
				state(entries != null, "No entries segment");
				Entry entryCurrent = entries.add(value);
				if (value instanceof ReferenceMapEntry) {
					((ReferenceMapEntry) value).setEntry(entryCurrent);
					return value;
				} else {
					return null;
				}
